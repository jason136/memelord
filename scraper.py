...
def main():
    ...
    for post in posts:
        ...
        if ext in image_exts:
            ...
            if (date_created not in dicts):
                if (os.path.exists(f'{memes_path}/{date_created}/{date_created}.csv')):
                    dicts[date_created] = pd.read_csv(f'{memes_path}/{date_created}/{date_created}.csv', usecols=keys).to_dict(orient='list')
                else:
                    dicts[date_created] = dict.fromkeys(keys)
                    for key in keys:
                        dicts[date_created][key] = []
                    if not os.path.exists(f'{memes_path}/{date_created}'):
                        os.makedirs(f'{memes_path}/{date_created}')
            ...
            filename = f'{memes_path}/{date_created}/{legal_filename}{ext}'
    ...
    for date, data in dicts.items():
        ...
        csv = dataframe.to_csv(f'{memes_path}/{date}/{date}.csv', index=True, header=True)
